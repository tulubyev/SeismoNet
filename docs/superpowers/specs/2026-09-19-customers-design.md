...
- Форма станции: `Select` региона (по умолчанию регион заказчика). Формы объекта в клиенте нет —
  создание/редактирование инфраструктурных объектов через UI не реализовано (уточнение 20.09);
  когда такая форма появится, в неё добавляется тот же селект региона.
