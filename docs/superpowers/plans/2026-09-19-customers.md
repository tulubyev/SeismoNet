...
- [ ] **Step 7: Verify** — `npx vitest run` green; `npm run check` ≤ 49 **plus** the new "missing customerId" errors that Tasks 5–7 will resolve are NOT acceptable — if `createStation`/seed call sites now fail to compile because `InsertStation` lost `customerId` while the table requires it, that is expected only for drizzle `.values()` calls; those live in storage create methods, which Tasks 5–6 rewrite. To keep this task green, in each affected storage create method add `customerId` temporarily as `(data as { customerId?: number }).customerId ?? 0`? **No** — instead do this: leave the storage create methods untouched in this task and accept a temporary tsc count above 49 **only** for errors mentioning `customerId` in `server/storage/*.ts` and `server/seed.ts`; report the exact count and the list. Restart the dev server (`pkill -f 'tsx server/index.ts'`, `npm run dev` in background) and confirm the startup log ends with `+ customers` and no migration error; through the tunnel: `SELECT code,name,region_id FROM customers` → `ecsem`, and `SELECT count(*) FROM stations WHERE customer_id IS NULL` → 0.
...
- Modify: `client/src/pages/InfrastructureObjects.tsx` (object form + create button), `client/src/pages/AddStation.tsx` (region select), `client/src/pages/{DevelopersPage or Developers}.tsx`, `SoilDatabase.tsx`, `Calculations.tsx`, `Seismograms.tsx`, `Sensors/Calibration` pages — every «Создать/Добавить» button
...
- [ ] **Step 3: "all" mode guards** — every create/add button on tenant pages: `const { canCreate } = usePermission();` → `disabled={!canCreate || …}` with `title="Выберите заказчика"` when `!canCreate`. Find them with `grep -rn "Создать\|Добавить" client/src/pages --include='*.tsx'` and cover: objects, developers, stations (`/stations/new` link), soil, calculations (save), seismograms (upload), sensors/installations, calibration sessions. In "all" mode object/station lists show a «Заказчик» column — the API rows carry `customerId`; map to names with the customers query (superadmin only).
