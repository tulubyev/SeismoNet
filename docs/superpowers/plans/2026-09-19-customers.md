...
- [ ] **Step 2: region selects** — `AddStation.tsx`: `Select` «Регион» from `/api/regions`, default `customer?.regionId`; send `regionId`, never `customerId`. (Ruled 20.09: the object half of this step is DROPPED — this codebase has no create/edit form for infrastructure objects, so there is nowhere to put the select. Building that form is a separate future feature.)
